...
caller's own job via a third-party decoder such as riscos_sprites --
see svg.py's wrap_single_sprite_as_area()), groups, and tagged objects
(recursing into both). Any other object type (text area, transformed
text/sprite, or anything unrecognised) is captured only as a bounding
box -- there is no pixel data or further structure decoded for it.
...
any particular application's own conversion source; the on-disk layout
here is verified against the official PRM file-formats reference
...
from riscos_drawfile import binary
...
class DrawSprite:
    ...
    identical to riscos_sprites' own single-sprite header layout, with
    none of a full multi-sprite SpriteArea file's own outer 12-byte
    area header (sprite_count/first_offset/free_offset) wrapped around
    it. A caller wanting real pixel data needs to synthesise that
    missing area header first -- see svg.py's
